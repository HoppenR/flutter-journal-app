...
class TagDayOverviewState extends State<TagDayOverview> {
    ...
    return Row(
      children: tagData.list.asMap().entries.map(
        (MapEntry<int, String> listEntry) {
          ...
          final bool isSelected = dateExists &&
              appliedTags[widget._date]!.any((AppliedTagData tag) =>
                  tag.name == tagData.name &&
                  tag.listOption == index);
          return TextButton(
            onPressed: () {
              _handleTagSelection(tagData, index, dateExists);
            },
  ...
  void _handleTagSelection(TagData tagData, int index, bool dateExists) {
    if (!dateExists) {
